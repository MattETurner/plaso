...
class PEParser(interface.FileObjectParser, dtfabric_helper.DtFabricHelper):
    ...
    _DEFINITION_FILE = os.path.join(os.path.dirname(__file__), "pe_resources.yaml")
    ...
    def _ParseMessageTableResource(
        self, parser_mediator, pefile_object, message_file, message_table_resource
    ):
    ...
    def _ParseWevtTemplate(self, parser_mediator, message_file, data):
        ...
        try:
            manifest = self._ReadStructureFromByteStream(data, 0, wevt_manifest_map)
        except (ValueError, errors.ParseError) as exception:
            raise errors.ParseError(
        ...
        for event_provider_descriptor in manifest.event_provider_descriptors:
            ...
            for (
                provider_element_descriptor
            ) in event_provider.provider_element_descriptors:
    ...
    def _ParseWevtTemplateResource(
        self, parser_mediator, pefile_object, message_file, wevt_template_resource
    ):
